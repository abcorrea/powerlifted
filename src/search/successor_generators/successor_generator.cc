...
&SuccessorGenerator::generate_successors(const std::vector<ActionSchema> &actions,
  ...
  for (const ActionSchema &action : actions) {
    ...
    for (size_t i = 0; i < action.get_positive_nullary_precond().size()
        and !trivially_inapplicable; ++i) {
      if ((action.get_positive_nullary_precond()[i] and !state.nullary_atoms[i])
          or (action.get_negative_nullary_precond()[i]
              and state.nullary_atoms[i])) {
    ...
    if (instantiations.tuples.empty()) {
      ...
      if (action.get_parameters().empty()) {
        // Action is ground
        bool applicable = is_ground_action_applicable(action,
                                                      state,
                                                      staticInformation);
        if (!applicable)
          continue;
        ...
        successors.emplace_back(State(move(new_relation),
                                      move(new_nullary_atoms)),
