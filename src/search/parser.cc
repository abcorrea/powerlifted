...
void parse_action_schemas(Task &task, int number_action_schemas) {
  ...
  for (int i = 0; i < number_action_schemas; ++i) {
    ...
    for (int j = 0; j < precond_size; ++j) {
      ...
      if (arguments_size==0) {
        assert(
            task.nullary_predicates.find(index)!=task.nullary_predicates.end());
        if (!negated)
      ...
      preconditions.emplace_back(move(precond_name),
                                 index,
                                 move(arguments),
                                 negated);
    }
    ...
    ActionSchema
        a(name, i, cost, parameters, preconditions, effects, inequalities,
          positive_nul_precond, negative_nul_precond,
          positive_nul_eff, negative_nul_eff);
    actions.push_back(a);
  }
...
void parse_initial_state(Task &task, int initial_state_size) {
  for (int i = 0; i < initial_state_size; ++i) {
    ...
    if (number_args==0) {
      assert(task.nullary_predicates.find(predicate_index)
                 !=task.nullary_predicates.end());
